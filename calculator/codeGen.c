#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "codeGen.h"

int isRegUsed[REGSIZE];
int preserveReg[REGSIZE];
void releaseReg(int i){
    isRegUsed[i] = 0;
}
void clearReg(){
    int i = 0;
    for(i = 0; i < REGSIZE; i++){
        if(isRegUsed[i] == 2) {
            isRegUsed[i] = 0;
        }
    }
}
int getAvailibleReg(int isVar){
    Symbol *reVar = NULL;
    int i = 0;
    for(i = 0; i < REGSIZE; i++){
        if(!isRegUsed[i]) {
            isRegUsed[i] = isVar ? 1 : 2;
            return i;
        }
    }
    // if no availible, release one
    reVar = leastVar();
    i = reVar->reg;
    printf("MOV [%d] r%d\n", reVar->mem, reVar->reg);
    reVar->reg = -1;
    return i;
}

int printAssembly_e(BTNode *root){
    if(root == NULL) return -1;
    int lr, rr;
    Symbol *var=NULL;

    switch (root->data) {
        case ID:
            root->reg = getAvailibleReg(0);
            var = Variable(root->lexeme);
            printf("MOV r%d [%d]\n", root->reg, var->mem);
            var->cnt--;
            break;
        case INT:
            root->reg = getAvailibleReg(0);
            printf("MOV r%d %d\n", root->reg, root->val);
            break;
        case ASSIGN:
            var = Variable(root->left->lexeme);
            rr = root->reg = printAssembly_e(root->right);
            printf("MOV [%d] r%d\n", var->mem, rr);
            break;
        case INCDEC:
            var = Variable(root->right->lexeme);
            rr = root->reg = printAssembly_e(root->right);
            lr = getAvailibleReg(0);

            printf("MOV r%d 1\n", lr);
            if (strcmp(root->lexeme, "++") == 0){
                printf("ADD r%d r%d\n", root->reg, lr);
            } else if (strcmp(root->lexeme, "--") == 0) {
                printf("SUB r%d r%d\n", root->reg, lr);
            }
            releaseReg(lr);
            printf("MOV [%d] r%d\n", var->mem, root->reg);
            break;
        case ADDSUB:
        case MULDIV:
        case AND:
        case XOR:
        case OR:
            lr = root->reg =  printAssembly_e(root->left);
            rr = printAssembly_e(root->right);
            if (strcmp(root->lexeme, "+") == 0) printf("ADD ");
            else if (strcmp(root->lexeme, "-") == 0) printf("SUB ");
            else if (strcmp(root->lexeme, "*") == 0) printf("MUL ");
            else if (strcmp(root->lexeme, "/") == 0) printf("DIV ");
            else if (strcmp(root->lexeme, "^") == 0) printf("XOR ");
            else if (strcmp(root->lexeme, "|") == 0) printf("OR ");
            else if (strcmp(root->lexeme, "&") == 0) printf("AND ");

            printf("r%d r%d\n", lr, rr);
            releaseReg(rr);
    }

    return root->reg;
}

// set reg, print assembly,set isVar
// return -1 if : !isVar, ID not registered
int printAssembly(BTNode *root, int use){
    if(root == NULL) return -1;
    int lr, rr;
    Symbol *var=NULL;

    switch (root->data) {
        case ID:
            var = Variable(root->lexeme);
            var->cnt--;
            if(!use){
                root->reg = -1;
            } else {
                root->reg = var->reg;
                if(root->reg == -1) {
                    root->reg = var->reg = getAvailibleReg(1);
                    if(!var->isVar) printf("MOV r%d %d\n", var->reg, var->val);
                    else printf("MOV r%d [%d]\n", var->reg, var->mem);
                }
            }
            break;
        case INT:
            if(!use){
                root->reg = -1;
            } else if(use){
                root->reg = getAvailibleReg(0);
                printf("MOV r%d %d\n", root->reg, root->val);
            }
            break;
        case ASSIGN:
            var = Variable(root->left->lexeme);
            var->cnt--;
            rr = printAssembly(root->right, 1);
            root->isVar = var->isVar = root->right->isVar;
            if(!root->isVar){
                
            }
            if(root->right->data==ID || root->right->data==ASSIGN || root->right->data==INCDEC){
                root->reg = var->reg = getAvailibleReg(1);
                printf("MOV r%d r%d\n", )
            } else {
                root->reg = var->reg = rr;
            }
            break;
        case INCDEC:
            var = Variable(root->right->lexeme);
            rr = printAssembly(root->right, 1);
            lr = getAvailibleReg(0);
            root->isVar = root->right->isVar;
            if(!use)
            printf("MOV r%d 1\n", lr);
            if (strcmp(root->lexeme, "++") == 0){
                printf("ADD r%d r%d\n", root->reg, lr);
            } else if (strcmp(root->lexeme, "--") == 0) {
                printf("SUB r%d r%d\n", root->reg, lr);
            }
            releaseReg(lr);
            printf("MOV [%d] r%d\n", var->mem, root->reg);
            break;
        case ADDSUB:
        case MULDIV:
        case AND:
        case XOR:
        case OR:
            lr = printAssembly(root->left, use);
            preserveReg[lr] = 1;
            rr = printAssembly(root->right, use);
            preserveReg[rr] = 1;
            root->isVar = root->left->isVar || root->right->isVar;

            if(!use){
                root->reg = -1;
            } else {
                root->reg = getAvailibleReg(0);
                if(!root->isVar){
                    if (strcmp(root->lexeme, "+") == 0) {
                        root->val = root->left->val + root->right->val;
                    } else if (strcmp(root->lexeme, "-") == 0) {
                        root->val = root->left->val - root->right->val;
                    } else if (strcmp(root->lexeme, "*") == 0) {
                        root->val = root->left->val * root->right->val;
                    } else if (strcmp(root->lexeme, "/") == 0) {
                        root->val = root->left->val / root->right->val;
                    } else if (strcmp(root->lexeme, "&") == 0) {
                        root->val = root->left->val & root->right->val;
                    } else if (strcmp(root->lexeme, "^") == 0) {
                        root->val = root->left->val ^ root->right->val;
                    } else if (strcmp(root->lexeme, "|") == 0) {
                        root->val = root->left->val | root->right->val;
                    }
                    printf("MOV r%d %d\n", root->reg, root->val);
                } else if(root->isVar){
                    if(root->left->data==ID || root->left->data==ASSIGN || root->left->data==INCDEC) {
                        root->reg = getAvailibleReg(0);
                    }
                    if (strcmp(root->lexeme, "+") == 0) printf("ADD ");
                    else if (strcmp(root->lexeme, "-") == 0) printf("SUB ");
                    else if (strcmp(root->lexeme, "*") == 0) printf("MUL ");
                    else if (strcmp(root->lexeme, "/") == 0) printf("DIV ");
                    else if (strcmp(root->lexeme, "^") == 0) printf("XOR ");
                    else if (strcmp(root->lexeme, "|") == 0) printf("OR ");
                    else if (strcmp(root->lexeme, "&") == 0) printf("AND ");

                    printf("r%d r%d\n", lr, rr);
                    releaseReg(rr);
                }
            }
            preserveReg[lr] = 0;
            preserveReg[rr] = 0;
            break;
    }
    return root->reg;
}

// Build table, cnt appearance, error NOTFOUND, error DIVZERO, set isVar, set val
int preprocess(BTNode *root){
    if(root == NULL) return;

    Symbol *var = NULL;
    int lv, rv;

    switch (root->data) {
        case ID:
            var = Variable(root->lexeme);
            if(var == NULL) 
                error(NOTFOUND);
            var->cnt++;
            root->isVar = 1;
            break;
        case INT:
            root->val = atoi(root->lexeme);
            root->isVar = 0;
            break;
        case ASSIGN:
            rv = preprocess(root->right);
            makeVariable(root->left->lexeme);
            if(rv) return 1;
            root->val = root->right->val;
            root->isVar = 0;
            break;
        case INCDEC:
            root->isVar = 1;
            rv = preprocess(root->right);
            break;
        case ADDSUB:
        case MULDIV:
        case AND:
        case XOR:
        case OR:
            lv = preprocess(root->left);
            rv = preprocess(root->right);
            if(!rv && root->right->val == 0 && strcmp(root->lexeme, "/")==0) error(DIVZERO);
            if(rv || lv) {
                root->isVar = 1;
                break;
            }
            
            if (strcmp(root->lexeme, "+") == 0) {
                root->val = root->left->val + root->right->val;
            } else if (strcmp(root->lexeme, "-") == 0) {
                root->val = root->left->val - root->right->val;
            } else if (strcmp(root->lexeme, "*") == 0) {
                root->val = root->left->val * root->right->val;
            } else if (strcmp(root->lexeme, "/") == 0) {
                root->val = root->left->val / root->right->val;
            } else if (strcmp(root->lexeme, "&") == 0) {
                root->val = root->left->val & root->right->val;
            } else if (strcmp(root->lexeme, "^") == 0) {
                root->val = root->left->val ^ root->right->val;
            } else if (strcmp(root->lexeme, "|") == 0) {
                root->val = root->left->val | root->right->val;
            }
            root->isVar = 0;
    }
    return root->isVar;
}

int evaluateTree(BTNode *root) {
    int retval = 0, lv = 0, rv = 0;

    if (root != NULL) {
        switch (root->data) {
            case ID:
                retval = Variable(root->lexeme)->val;
                break;
            case INT:
                retval = atoi(root->lexeme);
                break;
            case ASSIGN:
                rv = evaluateTree(root->right);
                retval = Variable(root->left->lexeme)->val = rv;
                break;
            case INCDEC:
                rv = evaluateTree(root->right);
                if(strcmp(root->lexeme, "++") == 0) retval = Variable(root->right->lexeme)->val = rv + 1;
                else if(strcmp(root->lexeme, "--") == 0) retval = Variable(root->right->lexeme)->val = rv - 1;
                break;
            case ADDSUB:
            case MULDIV:
            case AND:
            case XOR:
            case OR:
                lv = evaluateTree(root->left);
                rv = evaluateTree(root->right);
                if (strcmp(root->lexeme, "+") == 0) {
                    retval = lv + rv;
                } else if (strcmp(root->lexeme, "-") == 0) {
                    retval = lv - rv;
                } else if (strcmp(root->lexeme, "*") == 0) {
                    retval = lv * rv;
                } else if (strcmp(root->lexeme, "/") == 0) {
                    if (rv == 0)
                        error(DIVZERO);
                    retval = lv / rv;
                } else if (strcmp(root->lexeme, "&") == 0) {
                    retval = lv & rv;
                } else if (strcmp(root->lexeme, "^") == 0) {
                    retval = lv ^ rv;
                } else if (strcmp(root->lexeme, "|") == 0) {
                    retval = lv | rv;
                }
                break;
            default:
                retval = 0;
        }
    }
    return retval;
}

void printPrefix(BTNode *root) {
    if (root != NULL) {
        printf("%s ", root->lexeme);
        printPrefix(root->left);
        printPrefix(root->right);
    }
}

